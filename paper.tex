...
\footnotetext{Hedis provides another kind of context, \mintinline{haskell}{RedisTx}, for \emph{transactions}, united with \mintinline{haskell}{Redis} under the class of \mintinline{haskell}{RedisCtx}. We are demonstrating \mintinline{haskell}{Redis} only for brevity. }
...
Redis provides transactions, another context for executing commands.
Redis trasanctions are atomic in the sense that, all commands in a transaction
 ...
 be served in the middle.\footnotemark In contrast, we cannot make such
 guarantee in the ordinary context, which may destroy the assertions we made
 in types.
...
\footnotetext{\url{http://redis.io/topics/transactions}}
