...
in types. \mintinline{text}{DEL} removes a key from the current database,
...
In the type of \mintinline{haskell}{del}, the type varabile
...
To use \mintinline{haskell}{del}, we would have to apply it with a clunky
...
generic implementations for all datatypes with some language extenstions
enabled}
...
certain types of them. For example: \mintinline{haskell}{SET} with strings or
\mintinline{haskell}{SADD} with sets.
